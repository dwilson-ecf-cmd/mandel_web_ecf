...
средством запуска, а не производственной зависимостью. Формула владеет
инициализацией точки, рекуррентностью, правилом завершения и классификацией;
числовая реализация владеет арифметикой; вычисление владеет отображением
координат, циклом выполнения, счётчиками и записью поля. Визуализация владеет
растеризацией и представлением результата,
...
Манифест раздельно записывает идентичности формулы, её параметров, числовой
реализации, вычисления, визуализатора и памяти,
