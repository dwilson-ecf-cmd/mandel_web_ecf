# Краткий обзор восстановления соединения
...
Android и backend обновляются независимо: они согласуют опубликованные диапазоны контрактов и наборы возможностей. Разрыв сети не уничтожает Workspace: приложение заново соединяется, проходит authentication и выполняет reacquire у сервиса. Повторенная команда не выполняется дважды, потому что сервис хранит ограниченный replay-result cache. Кэшированный preview используется только при совпадении generation, identity и checksum. Адрес backend может измениться без изменения модели Workspace.
