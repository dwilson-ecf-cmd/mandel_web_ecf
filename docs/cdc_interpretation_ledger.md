...
| CDC-LEDGER-014 | For Experiment 0, values 0, 2, and 6 and the operations producing the trace are exactly representable under the documented binary64 assumptions. | `ENGINEERING_INTERPRETATION` | Experiment 0 finite-precision study | High | Keep this claim point-specific. |
| CDC-LEDGER-015 | The studied direct orbit mapping has a `NEGATIVE_RESULT`: proposed decreasing structures are circular, cosmetic, or unsupported and cannot certify the ordinary transitions as CDC steps. | `ENGINEERING_INTERPRETATION` | Experiment 0 mapping decision | High | Reopen only with a non-circular potential and terminal theorem. |
| CDC-LEDGER-016 | Study 0 uses the composite `(region, enclosure evidence, obligation, verified evidence)` as state; the region is claim subject and the obligation/evidence component changes. | `ENGINEERING_INTERPRETATION` | Object-of-descent decision; CDC-CONFIG-001 | Medium | Keep the claim limited to the fixed region and rule. |
| CDC-LEDGER-017 | A fixed one-claim proof deficit supplies a token potential `1 -> 0` when independent conventional evidence is verified and installed. | `ENGINEERING_INTERPRETATION` | CDC-STEP-001; CDC-CERT-001 | Medium | Reject passive storage, arbitrary schemas, or pre-encoded remaining work. |
| CDC-LEDGER-018 | Conventional interval/enclosure mathematics supplies the Study-0 guard and terminal escape theorem; CDC does not own those results. | `ENGINEERING_INTERPRETATION` | Ownership-boundary study | High | Preserve claim separation in validation. |
