...
#include <string.h>
const char *fractal_module_kind_string(fractal_module_kind k) { static const char *n[]={"formula","numeric","compute","refinement","scheduler","raster","encoder","memory","telemetry","platform"}; return (unsigned)k<FRACTAL_MODULE_KIND_COUNT?n[k]:NULL; }
fractal_result fractal_module_descriptor_validate(const fractal_module_descriptor *d,fractal_module_kind k) {
 ...
 for(const unsigned char *p=(const unsigned char *)d->module_id;*p;p++)if(*p<0x21u||*p>0x7eu)return FRACTAL_ERROR_INVALID_SPEC;
 return FRACTAL_OK;
}
