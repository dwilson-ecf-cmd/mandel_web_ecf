...
typedef enum fractal_module_kind { FRACTAL_MODULE_FORMULA=0, FRACTAL_MODULE_NUMERIC,
 ...
 FRACTAL_MODULE_TELEMETRY, FRACTAL_MODULE_PLATFORM, FRACTAL_MODULE_ARTIFACT_SINK,
 FRACTAL_MODULE_ANALYZER,
 FRACTAL_MODULE_KIND_COUNT } fractal_module_kind;
typedef struct fractal_module_descriptor { uint32_t abi_version, module_version;
 const char *module_id, *display_name; fractal_module_kind kind; uint64_t capability_flags; bool available;
...
enum { FRACTAL_CAP_SCALAR_ARITHMETIC=UINT64_C(1)<<0, FRACTAL_CAP_POINT_SCALAR=UINT64_C(1)<<1,
 ...
 FRACTAL_CAP_ENCODER_STREAM_OUTPUT=UINT64_C(1)<<6,
 FRACTAL_CAP_SINK_BINARY_BYTES=UINT64_C(1)<<7,
 FRACTAL_CAP_SINK_COMMIT_ABORT=UINT64_C(1)<<8,
 FRACTAL_CAP_SINK_MEMORY_DESTINATION=UINT64_C(1)<<9,
 FRACTAL_CAP_SINK_FILE_DESTINATION=UINT64_C(1)<<10,
 FRACTAL_CAP_ANALYZER_COMPATIBLE_FIELD=UINT64_C(1)<<11,
